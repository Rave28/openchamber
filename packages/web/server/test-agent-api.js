...
async function runTests() {
  ...
  if (health.status === 200 || health.status === 'ERROR') {
    log('warn', 'Server not running or /health endpoint not available');
  ...
  if (status.status === 200) {
    log('success', `Status endpoint working. Active agents: ${JSON.stringify(status.data)}`);
  } else {
    log('error', `Failed with status ${status.status}: ${JSON.stringify(status.data)}`);
  }
  console.log();
  ...
  if (status.status === 200) {
    ...
    log('error', `Failed with status ${status.status}: ${JSON.stringify(stats.data)}`);
  }
  console.log();
  ...
  if (spawn.status === 400 || spawn.status === 404) {
    ...
    log('warn', `Unexpected success with invalid path: ${JSON.stringify(spawn.data)}`);
  } else {
    log('error', `Unexpected status ${spawn.status}: ${JSON.stringify(spawn.data)}`);
  }
  console.log();
