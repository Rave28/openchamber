...
const normalizeThemeJson = (raw) => {
  ...
  const syntaxHighlights =
    syntax && typeof syntax === "object" ? syntax.highlights : null;
...
const readCustomThemesFromDisk = async () => {
  try {
    const entries = await fsPromises.readdir(OPENCHAMBER_USER_THEMES_DIR, {
      withFileTypes: true,
    });
    const themes = [];
    const seen = new Set();
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (stat.size > MAX_THEME_JSON_BYTES) {
          console.warn(
            `[themes] Skip ${entry.name}: too large (${stat.size} bytes)`,
          );
          continue;
        }
        ...
        if (seen.has(id)) {
          console.warn(
            `[themes] Skip ${entry.name}: duplicate theme id "${id}"`,
          );
          continue;
        }
...
const resolveWorkspacePath = (targetPath, baseDirectory) => {
  ...
  if (isPathWithinRoot(resolved, OPENCHAMBER_USER_CONFIG_ROOT)) {
    return {
      ok: true,
      base: path.resolve(OPENCHAMBER_USER_CONFIG_ROOT),
      resolved,
    };
  }
    ...
    for (const worktree of worktrees) {
      const candidate =
        typeof worktree?.worktree === "string"
          ? normalizeDirectoryPath(worktree.worktree)
          : "";
      if (!candidate) {
        continue;
      }
...
const resolveWorkspacePathFromContext = async (req, targetPath) => {
  ...
  if (!resolvedProject.directory) {
    return {
      ok: false,
      error: resolvedProject.error || "Active workspace is required",
    };
  }
  ...
  return resolveWorkspacePathFromWorktrees(
    targetPath,
    resolvedProject.directory,
  );
};

const normalizeRelativeSearchPath = (rootPath, targetPath) => {
  const relative =
    path.relative(rootPath, targetPath) || path.basename(targetPath);
...
const searchFilesystemFiles = async (rootPath, options) => {
  ...
  while (queue.length > 0 && candidates.length < collectLimit) {
    ...
    const dirResults = await Promise.all(
      batch.map(async (dir) => {
        if (!shouldRespectGitignore) {
          return {
            dir,
            dirents: await listDirectoryEntries(dir),
    ...
    for (const { dir: currentDir, dirents, ignoredPaths } of dirResults) {
      for (const dirent of dirents) {
        ...
        if (
          !entryName ||
          (!includeHiddenEntries && entryName.startsWith("."))
        ) {
          continue;
        }
        ...
        if (matchAll) {
          ...
          const score = fuzzyMatchScoreNormalized(
            normalizedQuery,
            relativePath,
          );
          if (score !== null) {
  ...
  return candidates
    .slice(0, limit)
    .map(({ name, path: filePath, relativePath, extension }) => ({
      name,
      path: filePath,
      relativePath,
      extension,
...
const readPushSubscriptionsFromDisk = async () => {
  try {
    ...
    if (
      ...
      parsed.version !== PUSH_SUBSCRIPTIONS_VERSION
    ) {
      return {
        version: PUSH_SUBSCRIPTIONS_VERSION,
        subscriptionsBySession: {},
    ...
    const subscriptionsBySession =
      ...
      typeof parsed.subscriptionsBySession === "object"
        ? parsed.subscriptionsBySession
        : {};
...
const writePushSubscriptionsToDisk = async (data) => {
  await fsPromises.mkdir(path.dirname(PUSH_SUBSCRIPTIONS_FILE_PATH), {
    recursive: true,
  });
  await fsPromises.writeFile(
...
const resolveProjectDirectory = async (req) => {
  ...
  if (projects.length === 0) {
    return {
      directory: null,
      error: "Directory parameter or active project is required",
    };
  }
...
const sanitizeSkillCatalogs = (input) => {
  ...
  for (const entry of input) {
    ...
    const subpath =
      typeof entry.subpath === "string" ? entry.subpath.trim() : "";
    const gitIdentityId =
      typeof entry.gitIdentityId === "string" ? entry.gitIdentityId.trim() : "";
...
const sanitizeProjects = (input) => {
  ...
  for (const entry of input) {
    ...
    const normalizedPath = rawPath
      ? path.resolve(normalizeDirectoryPath(rawPath))
      : "";
    const label =
      typeof candidate.label === "string" ? candidate.label.trim() : "";
    const addedAt = Number.isFinite(candidate.addedAt)
    ...
    if (
      candidate.worktreeDefaults &&
      typeof candidate.worktreeDefaults === "object"
    ) {
...
const sanitizeSettingsUpdate = (payload) => {
  ...
  if (Array.isArray(candidate.securityScopedBookmarks)) {
    result.securityScopedBookmarks = normalizeStringArray(
      candidate.securityScopedBookmarks,
    );
  }
  ...
  if (
    typeof candidate.markdownDisplayMode === "string" &&
    candidate.markdownDisplayMode.length > 0
  ) {
  ...
  if (typeof candidate.showTextJustificationActivity === "boolean") {
    result.showTextJustificationActivity =
      candidate.showTextJustificationActivity;
  }
  ...
  if (
    ...
    result.usageRefreshIntervalMs = Math.max(
      30000,
  ...
  if (
    ...
    const normalizedDays = Math.max(
      1,
      Math.min(365, Math.round(candidate.autoDeleteAfterDays)),
    );
  ...
  if (
    ...
    result.cornerRadius = Math.max(
      0,
      Math.min(32, Math.round(candidate.cornerRadius)),
    );
  }
  if (
    ...
    result.inputBarOffset = Math.max(
      0,
  ...
  if (
    typeof candidate.memoryLimitHistorical === "number" &&
    Number.isFinite(candidate.memoryLimitHistorical)
  ) {
    result.memoryLimitHistorical = Math.max(
      10,
      Math.min(500, Math.round(candidate.memoryLimitHistorical)),
    );
  }
  if (
    typeof candidate.memoryLimitViewport === "number" &&
    Number.isFinite(candidate.memoryLimitViewport)
  ) {
    result.memoryLimitViewport = Math.max(
      20,
      Math.min(500, Math.round(candidate.memoryLimitViewport)),
    );
  }
  if (
    typeof candidate.memoryLimitActiveSession === "number" &&
    Number.isFinite(candidate.memoryLimitActiveSession)
  ) {
    result.memoryLimitActiveSession = Math.max(
      30,
      Math.min(1000, Math.round(candidate.memoryLimitActiveSession)),
    );
  }
...
const mergePersistedSettings = (current, changes) => {
  ...
  if (
    typeof changes.lastDirectory === "string" &&
    changes.lastDirectory.length > 0
  ) {
    additionalApproved.push(changes.lastDirectory);
  }
  if (
    typeof changes.homeDirectory === "string" &&
    changes.homeDirectory.length > 0
  ) {
...
const validateProjectEntries = async (projects) => {
  console.log(
    `[validateProjectEntries] Starting validation for ${projects.length} projects`,
  );

  if (!Array.isArray(projects)) {
    console.warn(
      `[validateProjectEntries] Input is not an array, returning empty`,
    );
    return [];
  }
  ...
  const validations = projects.map(async (project) => {
    if (
      ...
      console.error(
        `[validateProjectEntries] Invalid project entry: missing or empty path`,
        project,
      );
      return null;
    }
    try {
      ...
      if (!stats.isDirectory()) {
        console.error(
          `[validateProjectEntries] Project path is not a directory: ${project.path}`,
        );
        return null;
      }
      return project;
      ...
      if (err && typeof err === "object" && err.code === "ENOENT") {
        console.log(
          `[validateProjectEntries] Removing project with ENOENT: ${project.path}`,
        );
        return null;
      }
      console.log(
        `[validateProjectEntries] Keeping project despite non-ENOENT error: ${project.path}`,
      );
      return project;
    }
  });
  ...
  console.log(
    `[validateProjectEntries] Validation complete: ${results.length}/${projects.length} projects valid`,
  );
  return results;
};
...
const migrateSettingsFromLegacyLastDirectory = async (current) => {
  ...
  if (nextProjects.length === 0) {
    const legacy =
      typeof settings.lastDirectory === "string"
        ? settings.lastDirectory.trim()
        : "";
  ...
  if (nextProjects.length > 0) {
    const active =
      nextProjects.find((project) => project.id === nextActiveProjectId) ||
      null;
    if (!active) {
...
const migrateSettingsFromLegacyThemePreferences = async (current) => {
  ...
  const themeId =
    typeof settings.themeId === "string" ? settings.themeId.trim() : "";
  const themeVariant =
    typeof settings.themeVariant === "string"
      ? settings.themeVariant.trim()
      : "";

  const hasLight =
    typeof settings.lightThemeId === "string" &&
    settings.lightThemeId.trim().length > 0;
  const hasDark =
    typeof settings.darkThemeId === "string" &&
    settings.darkThemeId.trim().length > 0;
...
const addOrUpdatePushSubscription = async (
  ...
  await persistPushSubscriptionUpdate((current) => {
    ...
    const existing = Array.isArray(subsBySession[uiSessionToken])
    ...
    return {
      version: PUSH_SUBSCRIPTIONS_VERSION,
      subscriptionsBySession: subsBySession,
    };
  });
};
...
const sendPushToAllUiSessions = async (payload, options = {}) => {
  ...
  await Promise.all(
    Array.from(subscriptionsByEndpoint.entries()).map(
      async ([endpoint, sub]) => {
...
const ensurePushInitialized = async () => {
  ...
  if (subject === "mailto:openchamber@localhost") {
    console.warn(
      "[Push] No public origin configured for VAPID; set OPENCHAMBER_VAPID_SUBJECT or enable push once from a real origin.",
    );
  }
...
const persistSettings = async (changes) => {
  ...
  persistSettingsLock = persistSettingsLock.then(async () => {
    console.log(
      `[persistSettings] Called with changes:`,
      JSON.stringify(changes, null, 2),
    );
    const current = await readSettingsFromDisk();
    console.log(
      `[persistSettings] Current projects count:`,
      Array.isArray(current.projects) ? current.projects.length : "N/A",
    );
    ...
    if (Array.isArray(next.projects)) {
      console.log(
        `[persistSettings] Validating ${next.projects.length} projects...`,
      );
      const validated = await validateProjectEntries(next.projects);
      console.log(
        `[persistSettings] After validation: ${validated.length} projects remain`,
      );
    ...
    if (Array.isArray(next.projects) && next.projects.length > 0) {
      const activeId =
        typeof next.activeProjectId === "string" ? next.activeProjectId : "";
      const active =
        next.projects.find((project) => project.id === activeId) || null;
      if (!active) {
        console.log(
          `[persistSettings] Active project ID ${activeId} not found, switching to ${next.projects[0].id}`,
        );
        next = { ...next, activeProjectId: next.projects[0].id };
      }
    } else if (next.activeProjectId) {
      console.log(
        `[persistSettings] No projects found, clearing activeProjectId ${next.activeProjectId}`,
      );
    ...
    await writeSettingsToDisk(next);
    console.log(
      `[persistSettings] Successfully saved ${next.projects?.length || 0} projects to disk`,
    );
...
function buildOpenCodeUrl(path, prefixOverride) {
  ...
  const prefix = normalizeApiPrefix(
    prefixOverride !== undefined ? prefixOverride : "",
  );
...
const setCachedSessionParentId = (sessionId, parentID) => {
  sessionParentIdCache.set(sessionId, {
...
const fetchSessionParentId = async (sessionId) => {
  ...
  try {
    ...
    const match = data.find(
      (s) => s && typeof s === "object" && s.id === sessionId,
    );
    const parentID =
      match && typeof match.parentID === "string" && match.parentID.length > 0
        ? match.parentID
        : null;
...
const maybeSendPushForTrigger = async (payload) => {
  ...
  if (payload.type === "message.updated") {
    ...
    if (info?.role === "assistant" && info?.finish === "stop" && sessionId) {
      ...
      if (settings.notifyOnSubtasks === false) {
        ...
        const parentIDFromPayload =
          sessionInfo?.parentID ?? payload.properties?.parentID;
  ...
  if (payload.type === "permission.asked" && sessionId) {
    ...
    const requestKey =
      typeof requestId === "string" ? `${sessionId}:${requestId}` : null;
...
async function restartOpenCode() {
  ...
  currentRestartPromise = (async () => {
    ...
    if (ENV_CONFIGURED_OPENCODE_PORT) {
      console.log(
        `Using OpenCode port from environment: ${ENV_CONFIGURED_OPENCODE_PORT}`,
      );
...
async function waitForOpenCodeReady(timeoutMs = 20000, intervalMs = 400) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (!configResult.ok) {
        lastError = new Error(
          `OpenCode config endpoint responded with status ${configResult.status}`,
        );
        await new Promise((resolve) => setTimeout(resolve, intervalMs));
        continue;
      }
      ...
      if (!agentResult.ok) {
        lastError = new Error(
          `Agent endpoint responded with status ${agentResult.status}`,
        );
        await new Promise((resolve) => setTimeout(resolve, intervalMs));
        continue;
      }
...
async function fetchAgentsSnapshot() {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch agents snapshot (status ${response.status})`,
    );
  }
...
async function fetchProvidersSnapshot() {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch providers snapshot (status ${response.status})`,
    );
  }
...
async function fetchModelsSnapshot() {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch models snapshot (status ${response.status})`,
    );
  }
...
function setupProxy(app) {
  ...
  app.use("/api", (req, res, next) => {
    ...
    const waitElapsed =
      openCodeNotReadySince === 0 ? 0 : Date.now() - openCodeNotReadySince;
    const stillWaiting =
      (!isOpenCodeReady &&
        (openCodeNotReadySince === 0 ||
  ...
  const proxyMiddleware = createProxyMiddleware({
    ...
    onProxyRes: (proxyRes, req, res) => {
      if (req.url?.includes("/event")) {
        console.log(
          `[SSE] Proxy response for ${req.method} ${req.url} - Status: ${proxyRes.statusCode}`,
        );
...
async function gracefulShutdown(options = {}) {
  ...
  const exitProcess =
    typeof options.exitProcess === "boolean"
      ? options.exitProcess
      : exitOnShutdown;
...
async function main(options = {}) {
  ...
  const onTunnelReady =
    typeof options.onTunnelReady === "function" ? options.onTunnelReady : null;
  ...
  app.get("/health", (req, res) => {
    res.json({
      ...
      openCodeRunning: Boolean(
        openCodePort && isOpenCodeReady && !isRestartingOpenCode,
      ),
      openCodeApiPrefix: "",
      openCodeApiPrefixDetected: true,
      isOpenCodeReady,
  ...
  app.get("/auth/session", (req, res) =>
    uiAuthController.handleSessionStatus(req, res),
  );
    ...
    if (origin.startsWith("http://") || origin.startsWith("https://")) {
      try {
        ...
        if (
          ...
          settings.publicOrigin.trim().length === 0
        ) {
  ...
  app.post("/api/push/visibility", (req, res) => {
    ...
    const visible =
      req.body && typeof req.body === "object" ? req.body.visible : null;
  ...
  app.post("/api/openchamber/update-install", async (_req, res) => {
    try {
      ...
      const { checkForUpdates, getUpdateCommand, detectPackageManager } =
  ...
  app.get("/api/openchamber/models-metadata", async (req, res) => {
    ...
    if (
      cachedModelsMetadata &&
      now - cachedModelsMetadataTimestamp < MODELS_METADATA_CACHE_TTL
    ) {
  ...
  app.get("/api/global/event", async (req, res) => {
    ...
    try {
      ...
      return res
        .status(502)
        .json({ error: "Failed to connect to OpenCode event stream" });
    }
...
`);
  ...
  app.get("/api/event", async (req, res) => {
    ...
    if (
      ...
      resolvedDirectory.trim().length > 0
    ) {
...
`);
  ...
  app.put("/api/config/settings", async (req, res) => {
    ...
    console.log(
      `[API:PUT /api/config/settings] Request body:`,
      JSON.stringify(req.body, null, 2),
    );
    try {
      const updated = await persistSettings(req.body ?? {});
      console.log(
        `[API:PUT /api/config/settings] Success, returning ${updated.projects?.length || 0} projects`,
      );
      res.json(updated);
    } catch (error) {
      console.error(
        `[API:PUT /api/config/settings] Failed to save settings:`,
        error,
      );
  ...
  app.get("/api/config/agents/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to get agent configuration metadata" });
    }
  });
  ...
  app.delete("/api/config/agents/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete agent" });
    }
  });
  ...
  app.get("/api/config/commands/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to get command configuration metadata" });
    }
  });
  ...
  app.post("/api/config/commands/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create command" });
    }
  });
  ...
  app.patch("/api/config/commands/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update command" });
    }
  });
  ...
  app.delete("/api/config/commands/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete command" });
    }
  });
  ...
  const { getCuratedSkillsSources } =
    await import("./lib/skills-catalog/curated-sources.js");
  const { getCacheKey, getCachedScan, setCachedScan } =
    await import("./lib/skills-catalog/cache.js");
  const { parseSkillRepoSource } =
    await import("./lib/skills-catalog/source.js");
  const { scanSkillsRepository } = await import("./lib/skills-catalog/scan.js");
  const { installSkillsFromRepository } =
    await import("./lib/skills-catalog/install.js");
  const { scanClawdHubPage, installSkillsFromClawdHub, isClawdHubSource } =
    await import("./lib/skills-catalog/clawdhub/index.js");
  const { getProfiles, getProfile } =
    await import("./lib/git-identity-storage.js");
  ...
  app.get("/api/config/skills/catalog", async (req, res) => {
    try {
      ...
      const settings = await readSettingsFromDisk();
      const customSourcesRaw =
        sanitizeSkillCatalogs(settings.skillCatalogs) || [];
    ...
    try {
      ...
      if (src.sourceType === "clawdhub" || isClawdHubSource(src.source)) {
        ...
        return res.json({
          ok: true,
          items,
          nextCursor: scanned.nextCursor || null,
        });
      }
      ...
      const effectiveSubpath =
        src.defaultSubpath || parsed.effectiveSubpath || null;
      ...
      return res.status(500).json({
        ...
        error: {
          kind: "unknown",
          message: error.message || "Failed to load catalog source",
        },
      });
    }
  });
  ...
  app.get("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to get skill configuration metadata" });
    }
  });
  ...
  app.post("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create skill" });
    }
  });
  ...
  app.patch("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update skill" });
    }
  });
  ...
  app.put("/api/config/skills/:name/files/*filePath", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to write skill file" });
    }
  });
  ...
  app.delete("/api/config/skills/:name/files/*filePath", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete skill file" });
    }
  });
  ...
  app.delete("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete skill" });
    }
  });
    ...
    if (!email) {
      try {
        ...
        const primaryVerified = list.find(
          (e) => e && e.primary && e.verified && typeof e.email === "string",
        );
        const anyVerified = list.find(
          (e) => e && e.verified && typeof e.email === "string",
        );
  ...
  app.get("/api/github/auth/status", async (_req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to get GitHub auth status" });
    }
  });

  app.post("/api/github/auth/start", async (_req, res) => {
    try {
      const { getGitHubClientId, getGitHubScopes, startDeviceFlow } =
        await getGitHubLibraries();
      const clientId = getGitHubClientId();
      if (!clientId) {
        return res.status(400).json({
          error:
            "GitHub OAuth client not configured. Set OPENCHAMBER_GITHUB_CLIENT_ID.",
        });
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to start GitHub device flow" });
    }
  });
  ...
  app.post("/api/github/auth/complete", async (req, res) => {
    try {
      ...
      if (!accessToken) {
        return res
          .status(500)
          .json({ error: "Missing access_token from GitHub" });
      }
  ...
  app.post("/api/github/auth/activate", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to activate GitHub account" });
    }
  });
  ...
  app.delete("/api/github/auth", async (_req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to disconnect GitHub" });
    }
  });
    ...
    try {
      ...
      try {
        ...
        if (error?.status === 401) {
          ...
          return res
            .status(401)
            .json({ error: "GitHub token expired or revoked" });
        }
        throw error;
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to fetch GitHub user" });
    }
  });
  ...
  app.get("/api/github/pr/status", async (req, res) => {
    try {
      ...
      if (!directory || !branch) {
        return res
          .status(400)
          .json({ error: "directory and branch are required" });
      }
      ...
      const { resolveGitHubRepoFromDirectory } =
        await import("./lib/github-repo.js");
      ...
      if (sha) {
        try {
          ...
          const checkRuns = Array.isArray(runs?.data?.check_runs)
      ...
      const mergedState = prData.merged
        ? "merged"
        : prData.state === "closed"
          ? "closed"
          : "open";
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to load GitHub PR status" });
    }
  });
  ...
  app.post("/api/github/pr/create", async (req, res) => {
    try {
      ...
      const head =
        typeof req.body?.head === "string" ? req.body.head.trim() : "";
      const base =
        typeof req.body?.base === "string" ? req.body.base.trim() : "";
      const body =
        typeof req.body?.body === "string" ? req.body.body : undefined;
      const draft =
        typeof req.body?.draft === "boolean" ? req.body.draft : undefined;
      if (!directory || !title || !head || !base) {
        return res
          .status(400)
          .json({ error: "directory, title, head, base are required" });
      }
      ...
      if (!repo) {
        return res
          .status(400)
          .json({ error: "Unable to resolve GitHub repo from git remote" });
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to create GitHub PR" });
    }
  });
    ...
    try {
      const directory =
        typeof req.body?.directory === "string"
          ...
          : "";
      const number =
        typeof req.body?.number === "number" ? req.body.number : null;
      const method =
        typeof req.body?.method === "string" ? req.body.method : "merge";
      if (!directory || !number) {
        return res
          .status(400)
          .json({ error: "directory and number are required" });
      }
      ...
      try {
        ...
        if (error?.status === 403) {
          return res
            .status(403)
            .json({ error: "Not authorized to merge this PR" });
        }
        if (error?.status === 405 || error?.status === 409) {
          return res.json({
            merged: false,
            message: error?.message || "PR not mergeable",
          });
        }
        throw error;
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to merge GitHub PR" });
    }
  });
    ...
    try {
      ...
      try {
        ...
        if (error?.status === 403) {
          return res
            .status(403)
            .json({ error: "Not authorized to mark PR ready" });
        }
        throw error;
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to mark PR ready" });
    }
  });
  ...
  app.get("/api/github/issues/list", async (req, res) => {
    try {
      ...
      const link =
        typeof list?.headers?.link === "string" ? list.headers.link : "";
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to list GitHub issues" });
    }
  });
  ...
  app.get("/api/github/issues/get", async (req, res) => {
    try {
        ...
        issue: {
          ...
          assignees: Array.isArray(issue.assignees)
            ? issue.assignees
                .map((u) =>
                  u
                    ? { login: u.login, id: u.id, avatarUrl: u.avatar_url }
                    : null,
                )
                .filter(Boolean)
            : [],
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to fetch GitHub issue" });
    }
  });
  ...
  app.get("/api/github/issues/comments", async (req, res) => {
    try {
      ...
      const comments = (Array.isArray(result?.data) ? result.data : []).map(
        (comment) => ({
          id: comment.id,
  ...
  app.get("/api/github/pulls/list", async (req, res) => {
    try {
      ...
      const prs = (Array.isArray(list?.data) ? list.data : []).map((pr) => {
        const mergedState = pr.merged_at
          ? "merged"
          : pr.state === "closed"
            ? "closed"
            : "open";
        ...
        return {
          ...
          headRepo:
            headRepo && headRepo.owner && headRepo.repo && headRepo.url
              ? headRepo
              : null,
        };
      });
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to list GitHub PRs" });
    }
  });
  ...
  app.get("/api/github/pulls/context", async (req, res) => {
    try {
      ...
      const includeCheckDetails =
        req.query?.checkDetails === "1" || req.query?.checkDetails === "true";
      if (!directory || !number) {
        return res
          .status(400)
          .json({ error: "directory and number are required" });
      ...
      const reviewComments = (
        ...
        position:
          typeof comment.position === "number" ? comment.position : null,
        author: comment.user
          ? {
              login: comment.user.login,
              id: comment.user.id,
      ...
      const files = (Array.isArray(filesResp?.data) ? filesResp.data : []).map(
        (f) => ({
          filename: f.filename,
          status: f.status,
            ...
            if (includeCheckDetails) {
              ...
              for (const runId of runIds) {
                try {
                  ...
                  const jobs = Array.isArray(jobsResp?.data?.jobs)
                    ? jobsResp.data.jobs
                    : [];
              ...
              if (includeCheckDetails && detailsUrl) {
                ...
                if (runId && Number.isFinite(runId)) {
                  ...
                  const matched = jobId
                    ...
                    : null;
                  const picked =
                    matched || jobs.find((j) => j.name === run.name) || null;
                  if (picked) {
                    job = {
                      runId,
                      jobId: picked.id,
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to load GitHub PR context" });
    }
  });
  ...
  app.get("/api/provider/:providerId/source", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get provider sources" });
    }
  });
  ...
  app.get("/api/quota/providers", async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to list quota providers" });
    }
  });
  ...
  app.delete("/api/provider/:providerId/auth", async (req, res) => {
    try {
      ...
      if (scope === "auth") {
        ...
        const projectRemoved = directory
          ? removeProviderConfig(providerId, directory, "project")
          : false;
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to disconnect provider" });
    }
  });
  ...
  app.post("/api/git/identities", async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Created git identity profile: ${profile.name} (${profile.id})`,
      );
      res.json(profile);
    } catch (error) {
  ...
  app.put("/api/git/identities/:id", async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Updated git identity profile: ${profile.name} (${profile.id})`,
      );
      res.json(profile);
    } catch (error) {
  ...
  app.get("/api/git/discover-credentials", async (req, res) => {
    try {
      const { discoverGitCredentials } =
        await import("./lib/git-credentials.js");
  ...
  app.post("/api/git/set-identity", async (req, res) => {
    const { getProfile, setLocalIdentity, getGlobalIdentity } =
      await getGitLibraries();
    try {
      const directory = req.query.directory;
      if (!directory) {
        return res
      ...
      if (profileId === "global") {
        ...
        if (!globalIdentity?.userName || !globalIdentity?.userEmail) {
          return res
            .status(404)
            .json({ error: "Global identity is not configured" });
        }
        profile = {
          id: "global",
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to set git identity" });
    }
  });
  ...
  app.get("/api/git/status", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get git status" });
    }
  });
  ...
  app.get("/api/git/diff", async (req, res) => {
    ...
    try {
      ...
      const context = req.query.context
        ? parseInt(String(req.query.context), 10)
        : undefined;
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get git diff" });
    }
  });
  ...
  app.get("/api/git/file-diff", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get git file diff" });
    }
  });
  ...
  app.post("/api/git/revert", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to revert git file" });
    }
  });
  ...
  app.post("/api/git/commit-message", async (req, res) => {
    ...
    try {
      ...
      if (diffs.length === 0) {
        return res
          .status(400)
          .json({ error: "No diffs available for selected files" });
      }
      ...
      if (!response.ok) {
        ...
        return res
          .status(502)
          .json({ error: "Failed to generate commit message" });
      }
      ...
      if (!raw) {
        return res
          .status(502)
          .json({ error: "No commit message returned by generator" });
      }

      const cleanedJson = stripJsonMarkdownWrapper(raw);
      const extractedJson =
        extractJsonObject(cleanedJson) || extractJsonObject(raw);
      const candidates = [cleanedJson, extractedJson, raw].filter(
      ...
      for (const candidate of candidates) {
        ...
        try {
          ...
          console.warn(
            "Commit message generation returned non-JSON body:",
            parseError,
          );
        }
      }
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to generate commit message" });
    }
  });
  ...
  app.post("/api/git/pr-description", async (req, res) => {
    ...
    try {
      ...
      if (diffs.length === 0) {
        return res
          .status(400)
          .json({ error: "No diffs available for base...head" });
      }
...
- head branch: ${head}`;
      ...
      if (!response.ok) {
        ...
        return res
          .status(502)
          .json({ error: "Failed to generate PR description" });
      }
      ...
      if (!raw) {
        return res
          .status(502)
          .json({ error: "No PR description returned by generator" });
      }
      ...
      for (const candidate of candidates) {
        ...
        try {
          ...
          console.warn(
            "PR description generation returned non-JSON body:",
            parseError,
          );
        }
      }
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to generate PR description" });
    }
  });
  ...
  app.post("/api/git/pull", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to pull from remote" });
    }
  });
  ...
  app.post("/api/git/push", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to push to remote" });
    }
  });
  ...
  app.post("/api/git/fetch", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to fetch from remote" });
    }
  });
  ...
  app.post("/api/git/commit", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create commit" });
    }
  });
  ...
  app.get("/api/git/branches", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get branches" });
    }
  });
  ...
  app.post("/api/git/branches", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create branch" });
    }
  });
  ...
  app.delete("/api/git/branches", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete branch" });
    }
  });
  ...
  app.put("/api/git/branches/rename", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to rename branch" });
    }
  });
  app.delete("/api/git/remote-branches", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to delete remote branch" });
    }
  });
  ...
  app.post("/api/git/checkout", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to checkout branch" });
    }
  });
  ...
  app.get("/api/git/worktrees", async (req, res) => {
    ...
    try {
      ...
      console.warn(
        "Failed to get worktrees, returning empty list:",
        error?.message || error,
      );
  ...
  app.post("/api/git/worktrees", async (req, res) => {
    ...
    try {
      ...
      const result = await addWorktree(directory, path, branch, {
        createBranch,
        startPoint,
      });
      res.json(result);
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to add worktree" });
    }
  });
  ...
  app.delete("/api/git/worktrees", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to remove worktree" });
    }
  });
  ...
  app.post("/api/git/ignore-openchamber", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update git ignore" });
    }
  });
  ...
  app.get("/api/git/worktree-type", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to determine worktree type" });
    }
  });
  ...
  app.get("/api/git/log", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get commit log" });
    }
  });
  ...
  app.get("/api/git/commit-files", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get commit files" });
    }
  });
  ...
  app.get("/api/fs/home", (req, res) => {
    try {
      ...
      if (!home || typeof home !== "string" || home.length === 0) {
        return res
          .status(500)
          .json({ error: "Failed to resolve home directory" });
      }
      res.json({ home });
    } catch (error) {
  ...
  app.get("/api/fs/read", async (req, res) => {
    ...
    try {
      ...
      if (resolvedPath.includes("..")) {
        return res
          .status(400)
          .json({ error: "Invalid path: path traversal not allowed" });
      }
      ...
      res
        .status(500)
        .json({ error: (error && error.message) || "Failed to read file" });
    }
  });
  ...
  app.post("/api/fs/write", async (req, res) => {
    ...
    try {
      ...
      await fsPromises.mkdir(path.dirname(resolved.resolved), {
        recursive: true,
      });
      ...
      res
        .status(500)
        .json({ error: (error && error.message) || "Failed to write file" });
    }
  });
  ...
  app.post("/api/fs/delete", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: (error && error.message) || "Failed to delete path" });
    }
  });
  ...
  app.post("/api/fs/rename", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: (error && error.message) || "Failed to rename path" });
    }
  });
  ...
  app.post("/api/opencode/directory", async (req, res) => {
    try {
      ...
      const existing =
        existingProjects.find((project) => project.path === resolvedPath) ||
        null;
      ...
      const activeProjectId = existing
        ? existing.id
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update working directory" });
    }
  });
  ...
  app.get("/api/fs/list", async (req, res) => {
    ...
    try {
      ...
      if (!stats.isDirectory()) {
        return res
          .status(400)
          .json({ error: "Specified path is not a directory" });
      }

      const dirents = await fsPromises.readdir(resolvedPath, {
        withFileTypes: true,
      });
      ...
      const isPlansPath =
        code === "ENOENT" &&
        (isPlansDirectory(resolvedPath) || isPlansDirectory(rawPath));
      if (!isPlansPath) {
  ...
  app.get("/api/fs/search", async (req, res) => {
    const rawRoot =
      typeof req.query.root === "string" && req.query.root.trim().length > 0
        ? req.query.root.trim()
        : typeof req.query.directory === "string" &&
            req.query.directory.trim().length > 0
          ? req.query.directory.trim()
          : os.homedir();
    ...
    const limitParam =
      typeof req.query.limit === "string"
        ...
        : undefined;
    const parsedLimit = Number.isFinite(limitParam)
      ? Number(limitParam)
    ...
    try {
      ...
      if (!stats.isDirectory()) {
        return res
          .status(400)
          .json({ error: "Specified root is not a directory" });
      }
      ...
      res
        .status(500)
        .json({ error: (error && error.message) || "Failed to search files" });
    }
  });
  ...
  app.post("/api/terminal/create", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create terminal session" });
    }
  });
  ...
  app.get("/api/terminal/:sessionId/stream", (req, res) => {
    ...
    res.write(
      `data: ${JSON.stringify({ type: "connected", runtime, ptyBackend })}\n\n`,
    );
    ...
    const dataHandler = (data) => {
      try {
        ...
        if (
          !ok &&
          session.ptyProcess &&
          typeof session.ptyProcess.pause === "function"
        ) {
          session.ptyProcess.pause();
          res.once("drain", () => {
            if (
              session.ptyProcess &&
              typeof session.ptyProcess.resume === "function"
            ) {
    ...
    console.log(
      `Terminal connected: session=${sessionId} client=${clientId} runtime=${runtime} pty=${ptyBackend}`,
    );
  });
  ...
  app.post(
    ...
    (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to write to terminal" });
      }
    },
  );
  ...
  app.post("/api/terminal/:sessionId/resize", (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to resize terminal" });
    }
  });
  ...
  app.delete("/api/terminal/:sessionId", (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to close terminal" });
    }
  });
    ...
    try {
      try {
        ...
        if (!stats.isDirectory()) {
          return res
            .status(400)
            .json({ error: "Invalid working directory: not a directory" });
        }
      } catch (error) {
        return res
          .status(400)
          .json({ error: "Invalid working directory: not accessible" });
      }
      ...
      console.log(
        `Restarted terminal session: ${sessionId} -> ${newSessionId} in ${cwd}`,
      );
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to restart terminal session" });
    }
  });
  ...
  try {
    ...
    if (await isOpenCodeProcessHealthy()) {
      console.log(
        `[HMR] Reusing existing OpenCode process on port ${openCodePort}`,
      );
    } else if (ENV_SKIP_OPENCODE_START && ENV_CONFIGURED_OPENCODE_PORT) {
      console.log(
        `Using external OpenCode server on port ${ENV_CONFIGURED_OPENCODE_PORT} (skip-start mode)`,
      );
      setOpenCodePort(ENV_CONFIGURED_OPENCODE_PORT);
      isOpenCodeReady = true;
      lastOpenCodeError = null;
  ...
  if (fs.existsSync(distPath)) {
    ...
    app.get(
      /^(?!\/api|.*\.(js|css|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot|map)).*$/,
      (req, res) => {
    ...
    app.get(
      ...
      (req, res) => {
        res
          .status(404)
          .send("Static files not found. Please build the application first.");
      },
    );
  }
      ...
      if (tryCfTunnel) {
        ...
        if (cfCheck.available) {
          try {
            ...
            cloudflareTunnelController = await startCloudflareTunnel({
              originUrl,
              port: activePort,
            });
            printTunnelWarning();
            ...
            console.error(
              `Failed to start Cloudflare tunnel: ${error.message}`,
            );
